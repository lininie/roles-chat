...
function searchUrlFor(name) {
  return 'http://www.baike.com/wiki/' + encodeURIComponent(name);
}
...
socket.on('servernotification', function (data) {
  ...
  if (data.connected) {
    if (data.toSelf) data.username = '你->' + name;
    $('#conversation').append('上线: <a href="' + searchUrl + '" target="_blank">' + escaped(data.username) + '</a><br/>');
  } else {
    $('#conversation').append('下线: <a href="' + searchUrl + '" target="_blank">' + escaped(data.username) + '</a><br/>');
  }
});
